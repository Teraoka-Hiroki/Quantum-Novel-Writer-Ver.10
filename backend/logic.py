...
class LogicHandler:
    ...
    def generate_candidates_api(api_key, topic_main, topic_sub1, topic_sub2, params, target_type=None):
        ...
        if target_type == "Scene Craft":
            ...
            instruction = f"""
            「Character Dynamics」（キャラクター造形）の執筆ブロックを１０個生成してください。各ブロック記事は１４０字程度にしてください。
            【重要方針】
            以下のパラメータ（0.0=弱い/少ない 〜 1.0=強い/多い）を反映した描写にしてください：
            ...
            instruction = "「Scene Craft」と「Character Dynamics」を合計１０個生成してください。"
    ...
    def generate_final(api_key, draft, instr):
        ...
        genai.configure(api_key=api_key)
        model = genai.GenerativeModel("gemini-2.5-flash")
        return LogicHandler._safe_generate(model, f"以下の指示に基づいて推敲してください: {instr}\n文章:\n{draft}")